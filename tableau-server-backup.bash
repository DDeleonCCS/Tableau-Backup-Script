...
###   Manual Run
#			  sudo su -l <tsmusername> -c /var/opt/tableau/tableau_server/scripts/tableau-server-backup.bash 
#
###		Scheduled Run: 
#       Run the script using cron to a regular basis
#			  sudo su -l $tsmuser -c "crontab -e"
#		
#	  Example: to schedule it to run once a day at 01:00, add this to your crontab
#		0 1 * * * /var/opt/tableau/tableau_server/scripts/tableau-server-backup.bash > /home/ddeleon/tableau-server-backup.log
#   BY DEFAULT Amazon EC2 Linux Instance goes by UTC; Schedule chron based on UTC
...
# Copy backups to DR Cluster
copy_backup="yes"
# Set the location to where backup are placed 
external_backup_path="/var/opt/tableau/tableau_server/data/tabsvc/files/backups/"
...
##### END OF LOAD ENVIRONMENT & USER INPUT #####
...
lastest_settings=$(ls -t $backup_path/*.json | head -n1)
#obtaining the latest backup
lastest_backup=$(ls -t $backup_path/*.tsbak | head -n1)

#copy backups to different location (optional)
if [ "$copy_backup" == "yes" ];
	then
	echo $TIMESTAMP "Copying backup and settings to remote share"
	# You can change where to send the backups to by changing the user: username@10.80.3.217
	scp $latest_settings ec2-user@10.80.3.217:$external_backup_path/
  scp $latest_backup ec2-user@10.80.3.217:$external_backup_path/
fi
